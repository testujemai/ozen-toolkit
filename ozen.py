...
if __name__ == '__main__':
    ...
    if not os.path.isfile('config.ini') and not args.ignore_cofnig:
        ...
        config['DEFAULT']['hf_token'] = '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'
        ...
        if file_ext in ['.mp3', '.m4a', '.flac', '.ogg', '.wav']:
            ...
            if args.mode == 'segment and transcribe':
                ...
                for wav in range(0,amount_to_train):
                    wav = str(wav)
                    res = transcribe_audio(os.path.join(wavs_path, base_name+'-'+wav+'.wav'),transcribe_pipe)[1:]
                    #add to train file
                    add_to_textfile(train_file, 'wavs/'+base_name+'-'+wav+'.wav|'+res+'\n')
                    pb.update(1)
                for wav in range(amount_to_train,amount_to_train+amount_to_valid):
                    wav = str(wav)
                    res = transcribe_audio(os.path.join(wavs_path, base_name+'-'+wav+'.wav'),transcribe_pipe)[1:]
                    #add to valid file
                    add_to_textfile(valid_file, 'wavs/'+base_name+'-'+wav+'.wav|'+res+'\n')
                    pb.update(1)
            ...
            if file_ext in ['.mp3', '.m4a', '.flac', '.ogg', '.wav']:
                ...
                if args.mode == 'segment and transcribe':
                    ...
                    for wav in range(0,amount_to_train):
                        gidx += 1
                        wav = str(wav)
                        res = transcribe_audio(os.path.join(wavs_path, base_name+'-'+str(gidx)+'.wav'),transcribe_pipe)[1:]
                        #add to train file
                        add_to_textfile(train_file, 'wavs/'+base_name+'-'+str(gidx)+'.wav|'+res+'\n')
                        pb.update(1)
                    for wav in range(amount_to_train,amount_to_train+amount_to_valid):
                        gidx += 1
                        wav = str(wav)
                        res = transcribe_audio(os.path.join(wavs_path, base_name+'-'+str(gidx)+'.wav'),transcribe_pipe)[1:]
                        #add to valid file
                        add_to_textfile(valid_file, 'wavs/'+base_name+'-'+str(gidx)+'.wav|'+res+'\n')
                        pb.update(1)
